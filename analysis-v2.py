...
class Net(nn.Module):
    ...
    def forward(self, x): # Dropout Batch Normalization and update weights with backpropagation
        x = F.relu(self.conv1d(x))
        x = x.view(-1, 16 * 9)  # Adjust input size for flattened data
        x = F.dropout(x, p=0.2) # dropout
        x = F.relu(self.fc1(x))
        x = F.dropout(x, p=0.2)
        x = F.relu(self.fc2(x))
        x = F.dropout(x, p=0.2)
...
def fig_training_loss(train_losses, test_losses):
    ...
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.savefig("model2_training_loss.png")
...
def fig_training_accu(train_accu, test_accu):
    ...
    plt.xlabel("Epochs")
    plt.ylabel("Accuracy")
    plt.savefig("model2_training_accuracy.png")
